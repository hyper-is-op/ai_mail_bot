...
def process_email_task(self, data):
    ...
    try:
        ...
        # ==============================
        cursor.execute(
            "SELECT id FROM paused_emails WHERE client_id = %s AND paused_email = %s",
            (client_id, data.get('from_email'))
        )
        if cursor.fetchone():
            ...
            """, (client_id, data.get('from_email'), data.get('subject'), data.get('body'),
                  'paused', 'Medium', 'Neutral', json.dumps(["Start", "Paused"])))
            db.commit()
            try:
                import os
                import redis
        ...
        if len(ticket_ids) > 1:
            ...
            send_email(data.get("client_id"), data["from_email"], "Re: " + data["subject"], clarification_reply)
            ...
            """, (
                data.get("client_id"), data["from_email"], data["subject"], data["body"],
                clarification_reply, 0, "clarification_sent", rag_id, sentiment, priority,
        ...
        if active_state_name:
            logger.info(f"🔒 Active verification state: {active_state_name} — PATHs A and B will be skipped")
            ...
            if rag_succeeded:
                ...
                if score >= threshold:
                    if features["feature_auto_send"]:
                        logger.info("✅ RAG reply quality good — auto_send")
                        send_email(client_id, data["from_email"], "Re: " + data["subject"], reply)
                        status = "sent"
                        execution_steps.append("SMTP_Send")
                        save_to_history = False
                    else:
                        logger.info("⏸ auto_send disabled — holding for manual approval")
                        status = "pending_manual_review"
                        execution_steps.append("Held_For_Manual_Review")
                        save_to_history = False
                else:
                    logger.warning("⚠️ RAG score below threshold — falling to PATH C")
                    execution_steps.append("Ticket_Escalation")
                    reply = None
                    score = 0

        # ──────────────────────────────
        # PATH C: RAG failed / low score / verification state machine.
        # ──────────────────────────────
        if reply is None:
...
"""
                    ...
                    push_message(client_id=client_id, from_email=data["from_email"], role="support",
                                 subject="Re: " + data["subject"], body=reply,
                                 ticket_id="", meta="")  # empty meta clears state
                    save_to_history = False
...
"""
        ...
        if save_to_history:
            push_message(
                client_id=client_id, from_email=data["from_email"], role="customer",
                subject=data["subject"], body=data["body"], ticket_id=""
            )
            push_message(
                client_id=client_id, from_email=data["from_email"], role="support",
                subject="Re: " + data["subject"], body=reply,
                ticket_id=outgoing_ticket_id or ""
            )
            logger.info("💬 Conversation saved to history")
        else:
            logger.info("⏭ History save skipped (already pushed or not needed)")
        ...
        cursor.execute("""
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (
            client_id, data["from_email"], data["subject"], data["body"],
            reply, score, status, rag_id, sentiment, priority,
            json.dumps(execution_steps)
        ))
        db_log_id = cursor.lastrowid
        generate_and_save_summary(db, cursor, db_log_id, data, chroma_context)
...
def _create_ticket_and_reply(data, client_id, context, history, cursor, sentiment="Neutral", priority="Medium"):
    ...
    resp = call_create_ticket(
        client_id, data["from_email"], data["subject"], data["body"],
        "Ticket_Generated", personal_details=personal_details
    )
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                outgoing_ticket_id, client_id,
                data.get("mail_id", f"msg-{outgoing_ticket_id}"),
                data["subject"], data["body"], "Ticket_Generated", sentiment, priority
            ))
    ...
    reply = generate_reply_llm(
        context=ticket_context, query=data["body"],
        agent_type="ecommerce_support_agent", from_email=data["from_email"],
        is_ticket=True, ticket_id=outgoing_ticket_id, history=history
    )

    send_status = send_email(
        client_id, data["from_email"],
        f"Ticket Update: {outgoing_ticket_id}", reply
    )
